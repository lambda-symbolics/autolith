...
(defun test-rlm-budget-accounting ()
  ...
  (let ((budget (rlm-budget-create :calls 2 :tokens 100 :depth 1)))
    ...
    (rlm-budget-acquire-call budget)
    (test-assert (= (rlm-budget-remaining-calls budget) 1)
                 "acquiring a call reserves it from the shared pool")
    (let ((tranche (rlm-budget-reserve-output budget)))
      (test-assert (= tranche 100)
                   "small pools reserve their whole remainder as the tranche")
      (test-assert (= (rlm-budget-remaining-tokens budget) 0)
                   "a reserved tranche leaves the shared pool")
                   ...
                   (rlm-budget-exhausted (condition)
                     (and (eq (rlm-budget-exhausted-dimension condition)
                          ...
                          (equal (rlm-budget-exhausted-task condition)
                                 "again")))
                   (error () nil))
                 "one remaining call admits exactly one reservation"))
  nil)
