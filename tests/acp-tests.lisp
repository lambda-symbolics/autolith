        ...
        (let ((messages (acp-test--read-updates output)))
          ...
          (let ((started (first messages)))
            ...
            (test-assert (equal (acp-test--update-field started "title")
                                "lisp.eval · (* 6 7)")
                           "started titles the tool with its form preview")
...
(defun test-acp-reasoning-batching ()
  "Small reasoning fragments coalesce into buffered thought chunks."
  (with-test-configuration (configuration)
    (let ((output (make-string-output-stream)))
      (let ((session (acp-test--tool-session configuration output)))
        (acp--session-reasoning session "thinking ")
        (acp--session-reasoning session "small text")
        (test-assert (null (acp-test--read-updates output))
                     "fragments below the flush length buffer silently")
        (acp--flush-reasoning session)
        (let ((messages (acp-test--read-updates output)))
          (test-assert (= (length messages) 1)
                       "the flush emits exactly one thinking chunk")
          (test-assert (equal (acp-test--update-field (first messages)
                                                      "sessionUpdate")
                              "agent_thought_chunk")
                       "the buffered batch arrives as a thought chunk")
          (let* ((entry (aref (acp-test--update-field (first messages)
                                                     "content")
                              0))
                 (body (json-get entry "content"))
                 (text (json-get body "text")))
            (test-assert (equal text "thinking small text")
                         "the buffered batch carries the joined fragments")))))
    (let ((output (make-string-output-stream)))
      (let ((session (acp-test--tool-session configuration output)))
        (acp--session-reasoning
         session (make-string 900 :element-type 'character
                                  :initial-element #\x))
        (let ((messages (acp-test--read-updates output)))
          (test-assert (= (length messages) 1)
                       "a fragment past the flush length notifies once"))))))
