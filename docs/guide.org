...
Autolith accepts an explicit Relay =plugins.toml= PluginConfig file and does not use
Relay's implicit =plugins.toml= discovery. The built-in =observability= component
is always allowed. Every other static component kind is rejected unless it is
named in =:allowed-component-kinds= as a custom observability component. Thus
Relay guardrails, pricing, and unrelated plugin components cannot be enabled
accidentally.
...
plugins. Native Relay discovery files at =${XDG_CONFIG_HOME}/nemo-relay/plugins.toml=,
=~/.config/nemo-relay/plugins.toml=, or =/etc/nemo-relay/plugins.toml= cause
startup to fail rather than being merged implicitly.
