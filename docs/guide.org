...
Autolith accepts an explicit Relay =plugins.toml= PluginConfig file. Native Relay
configuration discovery is disabled while Autolith observability is active, so
any discovered file is rejected before activation. The built-in =observability=
component is always allowed. Every other static component kind in Autolith's
explicit configuration is rejected unless it is named in
=:allowed-component-kinds= as a custom observability component.
...
[[components.config.opentelemetry.traces]]
type = "gen_ai"
...
extracts =plugin.id= and =plugin.kind= for its allowlist policy, then passes the
manifest reference and nested =config= table to Relay's dynamic activation API.
Relay validates the complete manifest, including compatibility, capabilities,
defaults, load metadata, and any integrity fields. Each manifest ID must also
be named in =:allowed-dynamic-plugin-ids= as trusted code approval:
...
plugins. Autolith requires an explicit PluginConfig. Native Relay discovery
files at =${XDG_CONFIG_HOME}/nemo-relay/plugins.toml=,
=~/.config/nemo-relay/plugins.toml=, or =/etc/nemo-relay/plugins.toml= are
rejected rather than merged when observability is enabled.
