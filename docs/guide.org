...
Windows x86-64 runs Autolith from a source checkout; there is no packaged
Windows release yet. Install Git for Windows, Rust with the MSVC toolchain
...
shell are recorded as skipped; the release scripts and the release server are
POSIX-only and are not built.
...
sandboxed command execution, detaching a session from its terminal and
client-first starts, checkpoints and rollback, and the =unix= management
REPL transport. Everything else, including local sessions over loopback TCP,
workers, the search engine, MCP, and skills, works as on POSIX hosts.
...
Commands run through =/bin/sh -c=; on Windows they run through Git for
Windows' =sh.exe= when it is installed and through PowerShell otherwise, and the
sandboxed execution choices are unavailable there.
