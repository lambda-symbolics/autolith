...
Autolith accepts an explicit Relay =plugins.toml= PluginConfig file. Relay then
applies its normal discovery and configuration merge rules. The built-in
=observability= component is always allowed. Every other static component kind
in Autolith's explicit configuration is rejected unless it is named in
...
plugins. Autolith requires an explicit PluginConfig; when it is supplied, native
Relay discovery files at =${XDG_CONFIG_HOME}/nemo-relay/plugins.toml=,
=~/.config/nemo-relay/plugins.toml=, or =/etc/nemo-relay/plugins.toml= may also
be discovered and merged by Relay.
